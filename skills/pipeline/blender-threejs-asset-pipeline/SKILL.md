...
Keep source editability separate from runtime grouping. Joining visible meshes by compatible material may be useful, but retain semantic aliases and the intended spatial granularity. Preserve prop pivots, identity and artwork across LODs; use [props](../../authoring/blender-threejs-props/SKILL.md). For newly required bitmap artwork, prefer the available image-generation tool and retain its editable source; export optimization must not replace the requested artwork or introduce floating glyph meshes.
